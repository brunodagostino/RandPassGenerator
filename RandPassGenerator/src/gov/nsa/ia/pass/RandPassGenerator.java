...
public class RandPassGenerator {
	...
	public RandPassGenerator(String logfile, PrintWriter pw, boolean verbose) {
		...
		try {
			...
			// 1.2 UPDATE: SP800-90A 8.6.7 states nonce should be a random value with at
			// least (security_strength/2) bits of entropy.
			// The value could be acquired from the same source and at the same time as the
			// entropy input (primarySource). Instead of saving entropy to a
			// file during shutdown for later use as the nonce entropy source or using the
			// system time when an entropy file doens't exist, use same entropy source
			// as primary entropy input for the DRBG seed. Also removes any concerns about
	...
	public int generateKeys(int count, int strength, boolean enc) {
		...
		for (i = 0; i < count; i++) {
			...
			if (rawkey == null) {
				...
				// hash the generated keys to use for KEY_ID and add to the key generation
				// transaction log
	...
	/**
	 * Key File Encryption Prompt. Encrypts generated key to a file using user
	 * inputed random password
	 */
	...
	/**
	 * Key ID Generator. Generates KeyID of key using first 64-bits of Hash and
	 * current date (YYYYMMDD_hhmmssH1H2H3H4H5H6H7H8H9H10H11H12H13H14H15H16).
	 */
	...
	private static OptionManager makeOptions() {
		...
		// ret.addOption("randfile", "Path to file to use for saved entropy", true,
